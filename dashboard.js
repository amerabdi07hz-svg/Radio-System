...
window.executeSearch = function(section) {
    ...
    dataSource.forEach(function(item) {
        var isMatch = terms.some(function(term) {
            ...
            if (section === 'devices') return (cleanArabic(item.serial || "").toLowerCase().includes(q) || cleanArabic(item.deviceName || "").toLowerCase().includes(q) || cleanArabic(item.deviceType || "").toLowerCase().includes(q) || cleanArabic(item.receiverName || "").toLowerCase() === q || cleanArabic(item.receiverName || "").toLowerCase().startsWith(q) || String(item.empId || "").toLowerCase() === q || String(item.empId || "").toLowerCase().startsWith(q) || cleanArabic(item.notes || "").toLowerCase().includes(q));
